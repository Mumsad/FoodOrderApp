...
import com.example.foodorder.databinding.CartItemBinding
            ...
            if (uniqueKey != null) {
                cartItemsReference.child(uniqueKey).removeValue()
                    .addOnSuccessListener {
                        // Check if the position is still valid and lists are not empty
                        if (position in cartItems.indices &&
                            position in cartItemPrices.indices &&
                            position in cartItemDescriptions.indices &&
                            position in cartImages.indices &&
                            position in cartItemQuantitys.indices &&
                            position in cartItemIngredients.indices
                        ) {
                            cartItems.removeAt(position)
                            cartItemPrices.removeAt(position)
